...
import { Cable, AlertTriangle, Loader2, MapPin, CheckCircle2, XCircle, HelpCircle, ExternalLink } from "lucide-react";
...
function FibreCoverageContent({ role }: { role: "admin" | "reseller" }) {
  ...
  useEffect(() => {
    function onMsg(e: MessageEvent) {
      if (e.data?.type === "coverage-result" && e.data.coverageType === "fibre") {
          ...
          {loading ? (
            ...
            <div className="p-10 flex flex-col items-center justify-center gap-3 text-center">
              ...
              <p className="text-sm text-muted-foreground max-w-sm">
                An admin must configure the Google Maps API key in Company Settings → API Keys to enable the fibre coverage map.
              </p>
            </div>
          ) : (
            <div className="p-4">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" onClick={() => setPopup(null)}>
          ...
          >
            ...
            <p className="text-sm text-muted-foreground text-center mb-4">
              {popup.status === "available"
                ? "Fibre coverage is available at this location."
                : popup.status === "not_available"
                ? "Fibre coverage is currently not available at this location."
                : "Verify fibre coverage on the map above or submit a formal request."}
            </p>
